...
const ArtistSpotifyItem = props => {
  ...
  return (
    <TouchableOpacity
      style={tw`bg-white m-3 p-4 rounded-lg`}
      onPress={() => {
        dispatchShowSearchModal(false);
        navigation.navigate('Artist', {artist: artist.item});
      }}>
      <Image
        source={{
          uri:
            artist.item.images[0] !== undefined
      ...
      />
      <View style={tw`mt-3`}>
        <Title title={artist.item.name} />
        <Text>{artist.item.type}</Text>
      </View>
    </TouchableOpacity>
  );
};

export default ArtistSpotifyItem;
